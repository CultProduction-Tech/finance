...
export interface BudgetMeta {
  ...
  /**
   * Имя из конфига нашлось, но у ДРУГОЙ записи PlanFact: бюджет завели заново под
   * прежним именем (Култ, 04.08.2026). Цифры верные — читаем то, что так названо, —
   * но закреплённый id указывает на прошлую версию, и запасной путь «нашли по id»
   * при следующем переименовании молча вернул бы её.
   * `pinnedTitle` — как сейчас называется та запись (null, если её уже нет).
   */
  reused?: { name: string; pinnedTitle: string | null }[] | null;
}
