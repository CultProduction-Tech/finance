...
const configs: Record<LegalEntity, EntityConfig> = {
  blaster: {
    ...
    amo: {
      ...
      // Завершённые (знаменатель Винрейта) — Продажа + Закрыто и не реализовано
      // (Реализовано убрано — это про выполнение работы, а не про принятие решения по сделке)
      conversionSoldStatusIds: [84825134, 143],
      // Победы — только "Продажа" (Реализовано убрано — это уже про выполнение, не про продажу)
      winStatusIds: [84825134],
